...
import { CheckCircle, XCircle, ShieldCheck, ShieldAlert, UserCheck, Eye, AlertTriangle } from 'lucide-react';
...
function AccessDenied() {
    return (
        <div className="flex flex-col items-center justify-center h-[50vh] gap-4 text-center">
            ...
            <p className="text-muted-foreground">هذه الصفحة مخصصة للأدمن فقط.</p>
             <Button asChild>
...
export default function AdminRequestsPage() {
  const { user } = useAuth();
  const { employeeRequests, updateRequestStatus } = useData();
  const [selectedRequest, setSelectedRequest] = useState(null);
  
  const adminPendingRequests = useMemo(() => {
    return employeeRequests.filter(req => req.status === 'قيد انتظار موافقة الأدمن');
  }, [employeeRequests]);


  const handleUpdateRequestStatus = (id: string, newStatus: 'approve' | 'reject') => {
      updateRequestStatus(id, newStatus, true); // true for admin action
  }
    ...
    <div className="flex flex-col gap-8">
      <header className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">اتخاذ الإجراء النهائي على الطلبات المعتمدة من المشرفين.</p>
        </div>
      </header>
       <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>الموافقة النهائية</AlertTitle>
          <AlertDescription>
            أنت في المرحلة النهائية من المراجعة. قرارك هنا نهائي.
          </AlertDescription>
        </Alert>
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription>قائمة بالطلبات التي تنتظر موافقتك النهائية.</CardDescription>
        </CardHeader>
        <CardContent>
            <Table>
                <TableHeader>
                ...
                <TableBody>
                {adminPendingRequests.length > 0 ? adminPendingRequests.map(req => (
                    <TableRow key={req.id}>
                    ...
                    <TableCell className="text-left">
                        {(req.status === 'قيد انتظار موافقة الأدمن') && (
                    ...
                    <TableRow>
                        <TableCell colSpan={5} className="text-center text-muted-foreground p-8">
                             لا توجد طلبات معلقة للموافقة النهائية حاليًا.
                        </TableCell>
                    </TableRow>
                )}
                </TableBody>
